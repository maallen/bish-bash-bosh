...
public class AuthResource {
	...
	public Response loginWithGoogle(@Valid final Payload payload, @Context final HttpServletRequest request)
		...
		return processUser(request, OAuthProvider.GOOGLE, userInfo);
	}
	...
	@POST
	@Path("/facebook")
	public Response loginWithFacebook(@Valid final Payload payload, @Context final HttpServletRequest request)
		...
		userInfo.put("picture", String.format(FACEBOOK_PROFILE_PIC_URL, id));

		// Step 3. Process the authenticated the user.
		return processUser(request, OAuthProvider.FACEBOOK, userInfo);
	}
	...
	@POST
	@Path("/signup")
	public Response signup(@Valid final User user, @Context final HttpServletRequest request) throws JOSEException, MongoDbException {
		...
		case 1:
			...
			final Response response = Response.status(Status.CONFLICT).entity(existingUser).build();
			return response;
		default:
	...
	private Response processUser(final HttpServletRequest request, final OAuthProvider provider, final Map<String, Object> userInfo) throws ParseException, JOSEException, MongoDbException {
		final BasicDBObject userQuery = new BasicDBObject().append("email", userInfo.get("email").toString());
		final DBCursor usersFoundInDB = mongoDBOperator.findDocumentsInCollection(userQuery, MongoDbCollection.USERS);
		...
		case 0:
			System.out.println("No records found for this user ==> Attempting to create user now and log in");
			addNewUserToDB(provider, userInfo, user);
			break;
		...
		case 1:
			...
			user.setPictureUrl(userInfo.get("picture").toString());
			break;
	...
	private void addNewUserToDB(final OAuthProvider provider, final Map<String, Object> userInfo, final User user) throws MongoDbException {
		switch(provider){
		case FACEBOOK:
			user.setProviderId(provider, userInfo.get("id").toString());
			break;
		case GOOGLE:
			user.setProviderId(provider, userInfo.get("sub").toString());
			break;
		case TWITTER:
		default:
			break;
		...
		user.setDisplayName(userInfo.get("name").toString());
		user.setPictureUrl(userInfo.get("picture").toString());

		final DBObject userDBObject = new BasicDBObject("email", userInfo.get("email").toString())
	...
	private void addNewNonOAuthUserToDB(final User user) throws MongoDbException {
